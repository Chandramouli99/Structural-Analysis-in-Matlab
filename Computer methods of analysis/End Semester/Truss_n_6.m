...
L=[3 3 3 3 4.242 4.242]; %length in m
A=[1 1 1 1 1 1]; %Area in m2
...
uul=[1 2 3 4]; %check labels of restrained and unrestrained dof
url=[5 6 7 8];
l1=[6 2 5 1]; %All globel labes once 
l2=[2 4 1 3];
l3=[4 8 3 7];
l4=[6 8 5 7];
l5=[6 4 5 3];
...
Tt1=zeros (4);
Tt2=zeros(4);
fem1=[0;0;0;0]; %check all free end moments 
fem2=[0;0;0;0];
fem3=[0;0;0;0];
fem4=[0;0;0;0];
fem5=[0;0;0;0];
...
for i=1:n %check n once 
    ...
    Ktotal=Ktotal + Knew;  % check wether total i=n or there or not  
        if i==1
            Tr1=T;
            Kg1=Kg;
            fembar1=Tt1'*fem1;
...
jl=[0;0;0;0;0;0;0;0];
jlu=[0;0;0;0]; %very important check carefully 
delu=KuuInv*jlu;
fprintf('Joint Load vector, Jl =\n');
disp(jl);
fprintf('displacements are Delu =\n');
disp(delu);
delr=[0;0;0;0]; %check this total no of zeros 
del=zeros(dof,1);
del=[delu;delr];
deli=zeros(4,1);
for i=1:n
    ...
    if i==1             %check wether i=n there or not 
        delbar1=deli;
